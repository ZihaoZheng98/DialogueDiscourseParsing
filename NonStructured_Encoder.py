...
class NonStructured_Encoder():
    def __init__(self, sess, FLAGS, embed, num_units=None, scope="Sentence_Encoder"):
        ...
        for var in tf.trainable_variables():
            ...
            if name.find(os.path.join(tf.contrib.framework.get_name_scope(), scope)) == 0:
    ...
    def initialize(self, vocab):
        op_in = self.symbol2index.insert(
            tf.constant(vocab), tf.constant(list(range(len(vocab))), dtype=tf.int64))
        op_out = self.index2symbol.insert(
            tf.constant(list(range(len(vocab))), dtype=tf.int64), tf.constant(vocab))
